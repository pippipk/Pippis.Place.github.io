...
var rectButton = new shapes(74,390,35, 50, colArray[0][0], colArray[4][2], "RECTANGLE"); // button to change shape to rectangle created
var elpButton = new shapes(117,400,50, 35, colArray[0][0], colArray[4][2], "ELLIPSE"); // button to change shape to ellipse is created
var triButton = new shapes(168,390,50, 50, colArray[0][0], colArray[4][2], "TRIANGLE"); // button for triangle is created
var circleButton = new shapes(15,390,50, 50, colArray[0][0], colArray[4][2], "CIRCLE"); // circle button created
var brushButton = new shapes(30,442,200, 60, colArray[0][0], colArray[4][2], "BRUSH"); // brush button created
var heartButton = new shapes(215,390,50, 50, colArray[0][0], colArray[4][2], "HEART"); // heart button is created

var box = new drawBUTTON(15,15,250,360, colArray[8][2], colArray[8][2]); // box for design feature is created to separate color buttons 
//from other buttons, I didn't hard code this so it can be moved or made bigger if more colours are added as buttons

var slide = new Slider(20,505, 240, 10,"white", "black"); // slider to change brush side is created
objectSet.push(draw, rectButton, elpButton, triButton, circleButton, brushButton, heartButton, box, slide);
...
for(var i=0; i<5; i++){
    ...
    if(i>1){ // the buttons for the undo, reset and redo are created
        var temp = new drawBUTTON(10+100*(i-2), 530, 90, 60, colArray[8][1], colArray[0][0], i+1);
    }
