class shapes{ // this is the class that creates the shape buttons to allow the user to draw different shapes on the canvas
    ...
    draw(){
        ...
        if(this.name=="BRUSH" | this.name=="LINE"){ // this draws an invisible button for the brush that allows the brush button to be selected
            ctx.rect(this.x, this.y, this.w, this.h);
        }
        if(this.name !== "BRUSH" && this.name!=="LINE"){
            ctx.fillStyle=this.col; // this fills all the buttons with colour except the brush and line button as this is invisible to the user under their respective images
            ctx.fill();
        }
    }
    ...
    update(){
        ...
        if(this.name == "LINE"){ // this draws the pencil image on the screen for the line button
            ctx.drawImage(imgObj2, this.x, this.y, this.wB, this.hB);
        }
        ...
        if(shapes.selected==this){ // if the brush button is selected the size will changed to show the user it is selected
            ...
            this.wB = this.wBoriginal; // when it is not selected the size will change back
...
class Slider{ // this is the class to create the slider which determines the line width
    constructor(x,y,w,h,col, col2){
        ...
        this.R = h/1.5; // this is the radius of the circle
